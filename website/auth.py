...
def login():
    ...
    if request.method == "POST":
        ...
        logger.debug(0, function, 'Recieved login request', **{'Login request recived from': _id})
        ...
        if user and check_password_hash(user.password, password):
            ...
            if user:
                logger.error(0, function, f'Password provided for user [{_id}] is wrong')
            else:
                logger.error(0, function, f'User [{_id}] does not exist in the database')
...
def signup():
    ...
    if request.method == "POST":
        ...
        logger.debug(0, function, f'Received Sign-Up request for user [{username}]', **signup_dump)

        user_username = get_user(username=username)
        user_email = get_user(email=email)

        if user_username:
            logger.error(0, function, f'Cannot add new user. User [{username}] already exists')
            flash("Username already exists.", category="error")
        elif user_email:
            logger.error(0, function, f'Cannot add new user. Email [{email}] already exists')
            flash("Email already exists.", category="error")
        elif len(email) < 4:
            logger.error(0, function, f'Cannot create account for user [{username}]. Email must be greater than 3 characters.')
            flash("Email must be greater than 3 characters.", category="error")
        elif len(full_name) < 2:
            logger.error(0, function, f'Cannot create account for user [{username}]. Name length is not valid')
            flash("First Name must be greater than 1 characters.", category="error")
        elif not gender:
            logger.error(0, function, f'Cannot create account for user [{username}]. Gender not specified')
            flash("Please specify your gender", category="error")
        elif password1 != password2:
            logger.error(0, function, f'Cannot create account for user [{username}]. Password and confirm pasword do not match.')
