...
def add_github():
    ...
    logger.debug(
        0, function, f"Attempting to add GitHub data for username [{username}] in DB"
    )
    try:
        ...
        logger.debug(
            0,
            function,
            f"Adding GitHub username [{username}] DB for user [{current_user.username}]",
        )
        ...
        logger.debug(
            0,
            function,
            f"Added GitHub username [{username}] for user [{current_user.username}]",
        )
        ...
        logger.error(
            0,
            function,
            f"Cannot add GitHub username [{username}] for user [{current_user.username}]. Error : [{e}]",
        )
...
@modify.route("/refresh_github", methods=["POST"])
@login_required
def refresh_github():
    try:
...
def add_codeforces():
    ...
    logger.debug(
        0,
        function,
        f"Adding CodeForces username [{username}] DB for user [{current_user.username}]",
    )
    ...
    try:
        ...
        users_collections.update_one(
            {"_id": current_user.username},
            {"$set": {"codeforces_username": username}},
            upsert=False,
        )
        ...
        logger.debug(
            0,
            function,
            f"Added CodeForces username [{username}] for user [{current_user.username}]",
        )
    except Exception as e:
        logger.error(
            0,
            function,
            f"Cannot add CodeForces username [{username}] for user [{current_user.username}]. Error : [{e}]",
        )
...
@modify.route("/refresh_codeforces", methods=["POST"])
@login_required
...
def add_codechef():
    ...
    logger.debug(
        0,
        function,
        f"Adding CodeChef username [{username}] DB for user [{current_user.username}]",
    )
    ...
    try:
        ...
        users_collections.update_one(
            {"_id": current_user.username},
            {"$set": {"codechef_username": username}},
            upsert=False,
        )
        ...
        logger.debug(
            0,
            function,
            f"Added CodeChef username [{username}] for user [{current_user.username}]",
        )
    except Exception as e:
        logger.error(
            0,
            function,
            f"Cannot add CodeChef username [{username}] for user [{current_user.username}]. Error : [{e}]",
        )
...
@modify.route("/refresh_codechef", methods=["POST"])
@login_required
...
def add_friend():
    ...
    logger.debug(
        0,
        function,
        f"[{current_user.username}] attempting to follow [{friend_username}]",
    )
    ...
    try:
        ...
        logger.debug(
            0,
            function,
            f"Added user [{friend_username}] to friend list of user [{current_user.username}]",
        )
        ...
        logger.error(
            0,
            function,
            f"User [{current_user.username}] cannot follow user [{friend_username}]. Error : [{e}]",
        )
...
def delete_friend():
    ...
    logger.debug(
        0,
        function,
        f"User [{current_user.username}] attempting to unfriend user [{friend_username}]",
    )
    try:
        users_collection = mongo.db.users
        users_collection.update_one(
        ...
        logger.debug(
            0,
            function,
            f"User [{current_user.username}] unfriended user [{friend_username}]",
        )
        ...
        logger.error(
            0,
            function,
            f"User [{current_user.username}] cannot unfriend user [{friend_username}]. Error : [{e}]",
        )
...
def add_upvote(friend_username):
    ...
    logger.debug(
        3,
        function,
        f"User [{current_user.username}] attempting to upvote user [{friend_username}]",
    )
    if current_user.username == friend_username:
    ...
    vote = votes_collection.find_one(
        {"current_username": current_user.username, "friend_username": friend_username}
    )
    is_voted = False
    if vote:
        is_voted = True
        if vote["type"] == "upvote":
            logger.debug(
                3,
                function,
                f"User [{current_user.username}] has already upvoted user [{friend_username}]. Removing upvote.",
            )
            remove_upvote(friend_username)
            logger.debug(
                3,
                function,
                f"Removed upvote of user [{current_user.username}] successfully from user [{friend_username}]",
            )
            return redirect(url_for("views.public_profile", username=friend_username))
    try:
        logger.debug(
            3,
            function,
            f"User [{current_user.username}] has not upvoted user [{friend_username}] yet. Adding upvote.",
        )
        ...
        users_collection.update_one(
            {"_id": friend_username},
            {"$inc": {"upvotes": 1, "downvotes": -1 if is_voted else 0}},
        )
        ...
        logger.debug(
            3,
            function,
            f"Added upvote from user [{current_user.username}] to user [{friend_username}]",
        )
        ...
        logger.error(
            0,
            function,
            f"Failed add upvote from user [{current_user.username}] to user [{friend_username}]. Error : [{e}]",
        )
...
def remove_upvote(friend_username):
    users_collection = mongo.db.users
    users_collection.update_one(
        {"_id": friend_username}, {"$inc": {"upvotes": -1}}, upsert=False
    )
...
def add_downvote(friend_username):
    ...
    logger.debug(
        3,
        function,
        f"User [{current_user.username}] attempting to downvote user [{friend_username}]",
    )

    if current_user.username == friend_username:
        logger.error(
            0, function, f"User [{current_user.username}] cannot downvote self"
        )
    ...
    if vote:
        ...
        if vote["type"] == "downvote":
            logger.debug(
                3,
                function,
                f"User [{current_user.username}] has already downnvoted user [{friend_username}]. Removing downvote.",
            )
            remove_downvote(friend_username)
            logger.debug(
                3,
                function,
                f"Removed downvote of user [{current_user.username}] successfully from user [{friend_username}]",
            )
            return redirect(url_for("views.public_profile", username=friend_username))
    try:
        logger.debug(
            3,
            function,
            f"User [{current_user.username}] has not downvoted user [{friend_username}] yet. Adding downvote.",
        )
        votes_collection.update_one(
            {
                "current_username": current_user.username,
        ...
        users_collection.update_one(
            {"_id": friend_username},
            {"$inc": {"downvotes": 1, "upvotes": -1 if is_voted else 0}},
        )
        ...
        logger.error(
            0,
            function,
            f"Failed add downvote from user [{current_user.username}] to user [{friend_username}]. Error : [{e}]",
        )
...
def remove_downvote(friend_username):
    users_collection = mongo.db.users
    users_collection.update_one(
        {"_id": friend_username}, {"$inc": {"downvotes": -1}}, upsert=False
    )
